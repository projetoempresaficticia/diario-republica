#!/usr/bin/env python3
"""Carimba as versões do Diário da República.
...
2. Escreve `versao.json` e o `<meta name="dr-versao">` de cada página,
...
META = re.compile(r'<meta name="dr-versao" content="(?P<versao>[^"]*)"\s*/?>')
...
META_RESUMO = re.compile(r'\s*<meta name="dr-versao"[^>]*>')
    ...
    for html in paginas:
        ...
        if atual:
            if atual.group('versao') != versao_site:
                problemas += 1
                texto = META.sub(
                    f'<meta name="dr-versao" content="{versao_site}" />', texto, count=1)
        else:
            problemas += 1
            print(f'  sem meta de versão: {html.name}')
            texto = texto.replace(
                '<meta name="viewport"',
                f'<meta name="dr-versao" content="{versao_site}" />\n  <meta name="viewport"',
                1)
        textos[html] = texto
